...
class DINO_Cine_Projector(nn.Module):
    def __init__(self, cfg, student_model, routing_module, mode="training", log_dir=None):
        ...
        self.norm_ehr = nn.LayerNorm(self.align_dim)
    ...
    def forward(self, x_temporal, x_spatial, prompt_text, ehr_feats, idx, case_id):
        ...
        img_token_reference = self.norm_img_ref(img_token_ref_unnormed)
        ...
        if self.mode != "training":
            ...
            student_out = self.student(x_temporal)[-2]
            ...
            visualize_and_save_attention(
                ...
                input_shape=(x_temporal.shape[2], x_temporal.shape[3], x_temporal.shape[4]),
        ...
        z_features_pooled = z_unpooled.mean(dim=-1)
        
        img_token_aligned = self.post_img_linear(z_features_pooled)
        img_token_aligned = self.norm_img(img_token_aligned)

        ehr_feats_squeezed = ehr_feats.squeeze(1)
        ...
        if not ehr_group_contributions:
            ...
            ehr_token = (ehr_group_contributions_cat * routing_weight.unsqueeze(-1)).sum(dim=1)
